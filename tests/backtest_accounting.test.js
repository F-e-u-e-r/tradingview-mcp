...
describe('accounting invariants', () => {
  ...
  it('exactly ONE approved MCP wiring path reaches the accounting layer (BT5 gate, MIGRATED)', () => {
    ...
    for (const rel of roots) {
      ...
      assert.ok(!code.includes('accounting'), `${rel} is not on the approved BT5 path — one path only (D8a)`);
    }
