...
export function registerAnalyticsTools(server, _deps) {
  server.registerTool('data_compute_indicator', {
    ...
    inputSchema: z.strictObject({
      ...
      timeframe: z.enum(['1', '5']).optional().describe('Optional timeframe CLAIM (owner amendment): OMIT for the served bars exactly as today. \'1\' computes from the canonical validated 1-minute snapshot (it does NOT independently assert the terminal 1-minute bar is completed); \'5\' derives analytics only from COMPLETED five-minute buckets of that same snapshot (Unix-epoch-aligned; metadata reports partial_leading_1m_bars / incomplete_terminal_1m_bars — neutral input-boundary counts: neither set enters derived 5m bars, though partial leading bars still seed the 5m VWAP\'s window-start anchor). Both require the chart at 1-minute resolution; 5-minute VWAP is derived from the 1-minute price-volume contributions, never recomputed from aggregated 5-minute OHLC.'),
    }),
